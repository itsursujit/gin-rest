...
func NewLogger() *logrus.Logger {
	...
	curDir, _ := os.Getwd()
	httpFile := time.Now().Format("http_log.2006-01-02")
	errorFile := time.Now().Format("error_log.2006-01-02")
	httpLogLocation := filepath.Join(curDir, "storage/logs/http/", httpFile + ".log")
	errorLogLocation := filepath.Join(curDir, "storage/logs/error/", errorFile + ".log")
	pathMap := PathMap{
		logrus.InfoLevel:  httpLogLocation,
		logrus.WarnLevel:  httpLogLocation,
		logrus.DebugLevel:  httpLogLocation,
		logrus.FatalLevel:  errorLogLocation,
		logrus.PanicLevel:  errorLogLocation,
		logrus.TraceLevel:  errorLogLocation,
		logrus.ErrorLevel: errorLogLocation,
	}
